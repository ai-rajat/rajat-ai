...
export default function MobileCatalogue() {
  ...
  if (activeHub) {
    ...
    return (
      <main className="min-h-screen bg-white text-gray-950 py-12 px-4 relative selection:bg-blue-600">
        <div className="mb-6">
          <button onClick={() => setActiveHub(null)} className="bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider shadow-md cursor-pointer">
            ← Master Hub
          </button>
        </div>
        <div className="text-center mb-10">
          <span className="bg-blue-100 text-blue-900 border border-blue-200 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">{titleText}</span>
          <h1 className="text-2xl font-black mt-3 text-gray-900">
            Select Layout to <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Preview</span>
          </h1>
          <p className="text-gray-600 text-xs mt-1">Tap on any card below to preview its exclusive layout.</p>
        </div>
        <div className="grid grid-cols-1 gap-4">
          {listData.map((item) => (
            <div 
              ...
              className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm hover:bg-blue-600 hover:border-blue-600 hover:text-white active:scale-[0.98] transition-all duration-300 cursor-pointer flex flex-col justify-between group"
            >
              <div>
                <span className="text-[10px] font-bold text-blue-900 bg-blue-100 group-hover:bg-blue-700 group-hover:text-white px-2.5 py-1 rounded-md inline-block mb-2 transition-colors">{item.badge}</span>
                <h3 className="text-lg font-bold text-gray-900 group-hover:text-white transition">{item.title}</h3>
                <p className="text-gray-600 text-xs mt-1 group-hover:text-blue-100 transition">{item.desc}</p>
              </div>
              <div className="mt-4 pt-3 border-t border-gray-100 group-hover:border-blue-500 flex items-center justify-between text-xs font-bold text-blue-600 group-hover:text-white transition-colors">
  ...
  // 3. Main Mobile Master Hub Landing
  return (
    <main className="min-h-screen bg-white text-gray-950 py-14 px-4 flex flex-col items-center justify-center selection:bg-blue-600 relative">
      {/* Back to Home Button */}
      <div className="absolute top-4 left-4">
        <Link href="/" className="bg-slate-900 text-white px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-wider shadow-md inline-flex items-center gap-1.5 cursor-pointer">
          ← Website
        </Link>
      </div>

      <div className="text-center mb-10 mt-6">
        <span className="bg-blue-100 border border-blue-200 text-blue-900 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest">
          RAJAT \ TECH • SHOWCASE
        </span>
        <h1 className="text-3xl font-black tracking-tight mt-3 mb-2 text-gray-900">
          Industry <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Showcase</span>
        </h1>
        <p className="text-gray-600 text-xs max-w-xs mx-auto">
          Tap a sector below to explore mobile layouts.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 w-full max-w-sm">
        <div 
          ...
          className="bg-white border border-gray-200 p-6 rounded-2xl cursor-pointer flex flex-col justify-between shadow-sm hover:bg-blue-600 hover:border-blue-600 hover:text-white active:scale-[0.98] transition-all duration-300 group"
        >
          <div>
            <span className="text-3xl mb-2 block">🛍️</span>
            <h2 className="text-lg font-bold mb-1 text-gray-900 group-hover:text-white transition">Store & Business</h2>
            <p className="text-gray-600 text-xs group-hover:text-blue-100 transition">Cafes, Boutiques, Gyms, Tech Repair</p>
          </div>
          <div className="mt-4 pt-3 border-t border-gray-100 group-hover:border-blue-500 flex items-center justify-between text-xs font-bold text-blue-600 group-hover:text-white transition-colors">
            <span>Open Hub</span>
            <span>→</span>
        ...
        >
          <div>
            ...
            <h2 className="text-lg font-bold mb-1 text-gray-900 group-hover:text-white transition">Real Estate Hub</h2>
            <p className="text-gray-600 text-xs group-hover:text-blue-100 transition">Villas, Condos, Penthouses</p>
          </div>
          <div className="mt-4 pt-3 border-t border-gray-100 group-hover:border-blue-500 flex items-center justify-between text-xs font-bold text-blue-600 group-hover:text-white transition-colors">
            <span>Open Hub</span>
            <span>→</span>
        ...
        >
          <div>
            <span className="text-3xl mb-2 block">⚡</span>
            <h2 className="text-lg font-bold mb-1 text-gray-900 group-hover:text-white transition">SaaS & AI Hub</h2>
            <p className="text-gray-600 text-xs group-hover:text-blue-100 transition">AI Copilots, Fintech, DevOps</p>
          </div>
          <div className="mt-4 pt-3 border-t border-gray-100 group-hover:border-blue-500 flex items-center justify-between text-xs font-bold text-blue-600 group-hover:text-white transition-colors">
            <span>Open Hub</span>
            <span>→</span>
        ...
        >
          <div>
            <span className="text-3xl mb-2 block">🎨</span>
            <h2 className="text-lg font-bold mb-1 text-gray-900 group-hover:text-white transition">Agency & Portfolio</h2>
            <p className="text-gray-600 text-xs group-hover:text-blue-100 transition">Creators, Developers, UI/UX</p>
          </div>
          <div className="mt-4 pt-3 border-t border-gray-100 group-hover:border-blue-500 flex items-center justify-between text-xs font-bold text-blue-600 group-hover:text-white transition-colors">
            <span>Open Hub</span>
            <span>→</span>
