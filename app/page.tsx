...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-600 selection:text-white flex flex-col">
      ...
      {/* NAVBAR: Changes dynamically based on device screen width */}
      <nav className="sticky top-0 z-50 flex items-center justify-between px-4 sm:px-8 py-4 sm:py-6 max-w-7xl mx-auto w-full bg-white/90 backdrop-blur-xl border-b border-slate-200/50 shadow-sm">
        <div className="text-xl sm:text-2xl font-black tracking-tighter text-slate-900 flex items-center gap-2.5">
        ...
        {isMobile ? (
          ...
          <>
            ...
            <a
              href="#contact"
              className="px-6 py-2.5 text-xs font-bold uppercase tracking-wider bg-slate-900 hover:bg-blue-600 text-white rounded-full transition-all duration-300 shadow-lg"
            >
              Work With Us
            </a>
          </>
      ...
      {isMobile && isOpen && (
        <div className="bg-slate-900 text-white px-6 py-6 flex flex-col gap-4 fixed inset-x-0 top-[73px] z-40 shadow-2xl border-b border-slate-800">
          <p className="text-[10px] font-bold tracking-widest text-blue-400 uppercase">Quick Navigation</p>
          <Link href="/services" onClick={() => setIsOpen(false)} className="text-base font-bold text-slate-200 hover:text-blue-400 py-1.5 border-b border-slate-800">
            ⚡ Services
          </Link>
          <Link href="/projects" onClick={() => setIsOpen(false)} className="text-base font-bold text-slate-200 hover:text-blue-400 py-1.5 border-b border-slate-800">
            🚀 Work / Projects
          </Link>
          <Link href="/team" onClick={() => setIsOpen(false)} className="text-base font-bold text-slate-200 hover:text-blue-400 py-1.5 border-b border-slate-800">
            👥 Leadership
          </Link>
          <Link href="/pricing" onClick={() => setIsOpen(false)} className="text-base font-bold text-slate-200 hover:text-blue-400 py-1.5 border-b border-slate-800">
            💎 Pricing
          </Link>
          <a
            href="#contact"
